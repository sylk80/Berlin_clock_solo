...
describe('Berlin Clock rows testing...', () => {
    ...
    test('fourth row should return YOOOOOOOOOO on 04:05:00...', () => {
        const clock = show_berlin_clock("04:05:00")
        let fourthRow = getNthRow(clock, 3)
        expect(fourthRow).toBe('YOOOOOOOOOO');
    });
    test('fourth row should return YYROOOOOOOO on 04:15:00...', () => {
        const clock = show_berlin_clock("04:15:00")
        let fourthRow = getNthRow(clock, 3)
        expect(fourthRow).toBe('YYROOOOOOOO');
    });
    test('fourth row should return YYRYYRYYRYY on 04:55:00...', () => {
        const clock = show_berlin_clock("04:55:00")
        let fourthRow = getNthRow(clock, 3)
        expect(fourthRow).toBe('YYRYYRYYRYY');
    });
    test('fifth row should return OOOO on 00:00:00...', () => {
        const clock = show_berlin_clock("00:00:00")
